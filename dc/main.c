...
#define P_1WireOUT P4OUT
#define P_1WireIN P4IN
#define DIR_P_1Wire P4DIR
...
void config_MSP430(void){
	...
	//ADC Msp430
	ADC12CTL0 = ADC12ON+MSC+SHT0_2 + REFON + REF2_5V;  // Turn on ADC12, set sampling time, 16 ADC12CLK cycles, Vref=1.5V
	ADC12CTL1 = SHP + CONSEQ_1+ADC12DIV2;
...
void outp(int bit){
	...
	DIR_P_1Wire |=BIT5;			//sets P1.0 as output

	if(bit == 0){
		P_1WireOUT= 0x0;           //drives P1.0 low
	}
	else{
		DIR_P_1Wire=0x00;             //releases the bus
	}
}
/************************************************************************/
/*  - name: inp
...
void Text(void){
  ...
  */ //ADC3
   ...
   b=b<<8;
   a=a+b;
   c=a*0.0003662109375;    //resolution
   //UART_TX(" ADC3= ");
   float_send(c);
   //UART_TX(" V\r\n");
  ...
  // UART_TX(" V\r\n");
   ...
   /* a=adc5_lsb;
    ...
    b=b<<8;
    a=a+b;
    c=(a*0.0003662109375)*2;    //resolution
    c=c/(133.25*0.062188545);
   // UART_TX(" ADC5= ");
    float_send(c);
    UART_TX(" V\r\n");
  ...
  // a=adc6_lsb;
    ...
    b=b<<8;
    a=a+b;
    c=(a*0.0006104 - 0.986)/0.00355;    //resolution
    UART_TX(" Temperature= ");
    float_send(c);
    UART_TX(" ºC\r\n");
